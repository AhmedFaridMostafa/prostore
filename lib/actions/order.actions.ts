...
export async function createOrder() {
  try {
    ...
    const insertedOrderId = await prisma.$transaction(async (tx) => {
      ...
      // Insert the order id in the order items
      const orderItemData = cart.items.map((item: CartItem) => ({
        ...item,
...
export async function getOrderById(orderId: string) {
  ...
  if (data?.user.email !== session.user.email && session.user.role !== "admin")
...
// Update Order to be paid by COD
